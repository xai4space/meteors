...
class DataReader(Dataset):
    ...
    def load_cropped_data(directory: str):
        ...
        for idx, file_name in tqdm(
            enumerate(all_files), total=len(all_files), desc="INFO: Data preloading for cropped augmentation.."
        ):
            # print(file_name)
            with np.load(file_name) as npz:
                flag = True
                ...
                for i in range(10):
                    ...
                    edge = (
                        11  # Randomly cropping the fields with 11x11 size, and adding some noise to the cropped samples
                    )
                    x = np.random.randint(sh[0] + 1 - edge)
                    y = np.random.randint(sh[1] + 1 - edge)
                    if (
                        np.sum(mask[0, x : (x + edge), y : (y + edge)]) > 120
...
class HyperDataloader:
    ...
    def __init__(
        self, train_root, label_path, test_root, im_size=224, split=0.20, batch_size=2, num_workers=1, target_index=None
    ):
        ...
        self.dataloaders["train"] = torch.utils.data.DataLoader(
            tr_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True
        )
