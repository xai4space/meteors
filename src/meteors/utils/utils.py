...
def agg_segmentation_postprocessing(soft_labels: bool = False, class_axis: int = 1) -> Callable[[torch.Tensor, torch.Tensor], torch.Tensor]:
    ...
    def postprocessing_function(output: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        max_classes = max(int(u.max().item()) for u in unique_classes)
        
        # Create a tensor to store the final counts for each class
        final_counts = torch.zeros(batch_size, max_classes+1)
