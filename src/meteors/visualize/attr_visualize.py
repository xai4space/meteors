...
def visualize_attributes(image_attributes: ImageAttributes, use_pyplot: bool = False) -> tuple[Figure, Axes] | None:
    """Visualizes the attributes of an image on the given axes.

    Parameters:
        image_attributes (ImageAttributes): The image attributes to be visualized.
    ...
    if image_attributes.image.orientation != ("C", "H", "W"):
        raise ValueError(f"Image orientation {image_attributes.image.orientation} is not supported yet")
    ...
    fig.suptitle(f"Image Attributes of: {image_attributes.attribution_method}")
    ...
    ax[1, 0].scatter(image_attributes.image.wavelengths, attr_all, c="r")
    ...
    ax[1, 1].scatter(image_attributes.image.wavelengths, attr_abs, c="b")
    ...
    ax[1, 2].scatter(image_attributes.image.wavelengths, sign_attr, c="g")
