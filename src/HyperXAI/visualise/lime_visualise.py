...
def visualise_spectral_attributes(spectral_attributes: HyperImageSpectralAttributes | List[HyperImageSpectralAttributes], use_pyplot=False, color_palette = None, show_not_included = False):
    ...
    fig.suptitle("Lime attribution")
    visualise_spectral_attributes_by_waveband(spectral_attributes, ax[0], color_palette = color_palette, show_not_included = show_not_included)
    
    visualise_spectral_attributes_by_magnitude(spectral_attributes, ax[1], color_palette = color_palette, show_not_included = show_not_included)
...
def visualise_spectral_attributes_by_waveband(spectral_attributes: HyperImageSpectralAttributes | List[HyperImageSpectralAttributes], ax: matplotlib.axes.Axes | None, color_palette = None, show_not_included = True) ->  matplotlib.axes.Axes:
    ...
    if not isinstance(spectral_attributes, HyperImageSpectralAttributes):
        ...
        attribution_map = torch.zeros(len(spectral_attributes), len(spectral_attributes[0].get_flattened_attributes()))
    ...
    for idx, band_name in enumerate(band_names.keys()):
            ...
            if aggregate_results:
                ax.errorbar(wavelengths[flattened_band_mask == segment_id], 
                ...
                ax.scatter(wavelengths[flattened_band_mask == segment_id],
                            attribution_map[:, flattened_band_mask == segment_id].mean(dim=0),
                            label=band_name, s=50, color=color_palette[idx])  # Increased marker size
