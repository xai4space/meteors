...
class HyperImage(BaseModel):
    ...
    orientation: Annotated[Sequence[str], Field(kw_only=False, validate_default=True, description='orientation of the image - sequence of three one-letter strings in any order: "C", "H", "W" meaning respectively channels, height and width of the image. Defaults to ("C", "H", "W")')] = ("C", "H", "W")
    ...
    def validate_orientation(cls, value):
        assert len(value) == 3, "Orientation should be a sequence of 3 elements: 'W', 'H' and 'C' in order of the image dimensions"
        assert "H" in value and "W" in value and "C" in value, "Orientation should contain 'W', 'H' and 'C' in any order"
        return value
    
    @field_validator("image", mode="before")
    @classmethod
    def validate_image(cls, image):
        assert len(image.shape) == 3, "Image should have 3 dimensions: channels, height and width"
        if not isinstance(image, torch.Tensor):
            assert isinstance(image, np.ndarray), "Image should be a numpy array or torch tensor"
    ...
    def validate_band_axis(self) -> Self:
        ...
        assert len(self.wavelengths) == self.image.shape[self.band_axis], "Number of wavelengths should be equal to the number of bands in the image"
        return self
    
    @model_validator(mode = "after")
    def validate_binary_mask(self) -> Self:
        if not isinstance(self.binary_mask, torch.Tensor) and self.binary_mask is not None and not isinstance(self.binary_mask, str):
        ...
        if isinstance(self.binary_mask, str) and self.binary_mask == "artificial":
            self.binary_mask = torch.index_select(self.image, 0, torch.tensor([0], device=self._device)).bool()[0] # get the first channel and encode it to bools
            self.binary_mask = torch.repeat_interleave(torch.unsqueeze(self.binary_mask, dim=self.band_axis), repeats=self.image.shape[self.band_axis], dim=self.band_axis)

        assert self.binary_mask is None or self.binary_mask.shape == self.image.shape, "Binary mask should have the same shape as the image"        
        return self  
    ...
    def get_rgb_image(self, mask = True, cutoff_min=False, output_band_index: int = None) -> torch.tensor:
        """get an RGB image from hyperspectral image. Usefull for plotting
    ...
    def get_flattened_binary_mask(self) -> torch.tensor:
        ...
        transposed_binary_mask = self.binary_mask if self.band_axis == 2 else torch.moveaxis(self.binary_mask, self.band_axis, 2) # move channel axis to the back
    ...
    def select_single_band_from_name(self, band_name, method="center", mask = True, cutoff_min=False, normalize = True):
        ...
        if method == "center":
            ...
            if self.binary_mask is not None and mask:
                transposed_binary_mask = self.binary_mask if self.band_axis == 2 else torch.moveaxis(self.binary_mask, self.band_axis, 2) # move channel axis to the back
            transposed_image = self.image if self.band_axis == 2 else torch.moveaxis(self.image, self.band_axis, 2) # move channel axis to the back
    ...
    def _get_rgb_img(self, mask = True, cutoff_min=False) -> torch.tensor:
        return torch.stack([self.select_single_band_from_name(band, normalize = True, mask = mask, cutoff_min=cutoff_min) for band in ["R", "G", "B"]], axis=self.band_axis)
