...
def test_validate_consistent_band_and_wavelengths():
    ...
    spectral_attributes = [
        ...
        ImageSpectralAttributes(
            ...
            attributes=attributes,
            score=score,
            band_names=inconsistent_band_names,
            band_mask=band_mask
        ),
    ]
    ...
    with pytest.raises(ValueError):
        visualize.validate_consistent_band_and_wavelengths(inconsistent_band_names, torch.tensor(wavelengths_main), spectral_attributes)
    ...
    with pytest.raises(ValueError):
        visualize.validate_consistent_band_and_wavelengths(band_names, torch.tensor(inconsistent_wavelengths), spectral_attributes)
...
def test_setup_visualization():
    ...
    # Test with None axes
    result_ax = visualize.setup_visualization(None, "Test Title 2", "Y Label", "X Label",)
...
def test_visualize_spectral_attributes_by_waveband():
    ...
    # Cleanup
    ax.clear(); fig.clear(); plt.close("all")
    del ax, fig
