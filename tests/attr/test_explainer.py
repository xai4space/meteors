...
import meteors.attr.explainer as explainer
...
def test_validate_attribution_method_initialization(explainable_toy_model):
    ...
    attribution_method = explainer.Explainer(explainable_toy_model)
    explainer.validate_attribution_method_initialization(attribution_method)
    ...
    not_working_model = explainer.Explainer(ExplainableModel(problem_type="regression", forward_func=None))
    with pytest.raises(ValueError):
        explainer.validate_attribution_method_initialization(not_working_model)
...
def test_validate_and_transform_baseline():
    ...
    with pytest.raises(ValueError):
        explainer.validate_and_transform_baseline(baseline, hsi)
...
def test_explainer():
    ...
    explainer = explainer.Explainer(explainable_model)
    ...
    chained_explainer = explainer.Explainer(explainer)

    # chained explainer with chained explainer - should raise ValueError
    with pytest.raises(ValueError):
        explainer.Explainer(chained_explainer)
